...
export default function RepoInfoCard({ workspace, repo }: RepoInfoCardProps) {
  ...
  const [stats, setStats] = useState<{ files: number; branches: number; commits: number; pullRequests: number } | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`/api/repos/stats?${new URLSearchParams({ workspace, repo }).toString()}`)
      .then((r) => r.json())
      ...
      >
        ...
        {stats && (
          <span className="hidden sm:inline truncate text-[var(--text-tertiary)]">
            {stats.files} dosya · {stats.commits} commit · {stats.branches} branch · {stats.pullRequests} PR
          </span>
        )}
      </button>
            ...
            <div className="grid grid-cols-4 gap-2 mb-3">
              ...
              {stats && (
                <>
                  ...
                  <div className="px-2 py-1.5 rounded-lg bg-[var(--bg-tertiary)] text-center">
                    <p className="text-[10px] text-[var(--text-tertiary)]">PR</p>
                    <p className="text-[12px] text-[var(--text-primary)] font-medium">{stats.pullRequests}</p>
                  </div>
                </>
              )}
            </div>
